...
export default function RegisterForm({ onToggleMode }: RegisterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      body.append("userType", formData.userType);
      ...
      const response = await fetch('/api/auth/register', {
        method: 'POST',
        body, // ⬅️ NO JSON HEADERS — FormData automatically sets correct headers
      });
            ...
            <div className="space-y-2">
              ...
              {imagePreview && (
                <img
                  src={imagePreview}
                  alt="Preview"
                  className="w-20 h-20 rounded-full border object-cover mx-auto"
                />
              )}
            </div>
